...
function Sidebar() {
    ...
    const menuItems = [
        ...
        {to: "/saving", icon: "saving.svg", label: tSidebar.saving, hasArrow: true,modal:"depositModal",path:"depositModal"},
    ...
    const handleOpenModal = (e, modal, path) => {
        ...
        if (activeModal === modal) {
            ...
            navigate(path, {replace: true}); // Навигация по маршруту
        }
    };
                    ...
                    {menuItems.map(({to, icon, label, hasArrow, modal}) => (
                        <li key={to}>
                            {modal ? (
                                <button>
                                    <div
                                        className={`${styles.iconContainer} ${activeModal === modal ? styles.active : ""}`}
                                        onClick={(e) => handleOpenModal(e, modal,to)}>
